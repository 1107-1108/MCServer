    ...
    if (Object.keys(LUCKY_EVENTS).indexOf(block.id) != -1) {
        ...
        if ('kubejs:lucky_gem_pickaxe' == player.handSlots[0].id) {
            server.scheduleInTicks(20 * 1, () => {
                server.runCommandSilent(server.runCommandSilent(`execute in ${player.level.dimension} run fill ${Math.round(block.x)} ${Math.round(block.y)} ${Math.round(block.z)} ${Math.round(block.x)} ${Math.round(block.y)} ${Math.round(block.z)} ${block.id}`))
            })
        } else if ('kubejs:lucky_gem_pickaxe_imitation' == player.handSlots[0].id) {
