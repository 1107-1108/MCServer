    ...
    if (entity.isPlayer()) {
        ...
        for (let key of config) {
            if (difficulty > parseFloat(key)) {
                server.runCommandSilent(`improvedmobs difficulty player ${entity.displayName.string} add -${AUTO_DIFFICULTY_REDUCE[key]}`);
                player.tell(Text.red(Text.translate('message.kubejs.difficulty.reduce', [AUTO_DIFFICULTY_REDUCE[key]])));
                break;
            }
        }
    }
